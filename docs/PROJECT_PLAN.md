...
| 20 Aug 2026 | Identifiers detected by **digit run then classification**, not by exact length (§7G) | Length-anchored patterns missed every unlisted format and, worse, produced partial matches: `4111 1111 1111 11119` had twelve digits masked as an Aadhaar number and `11119` written in cleartext, so the output looked redacted while leaking. Consuming the whole run first makes a partial match structurally impossible. Unrecognised lengths become `id_number` — over-redaction is the safe direction. |
| 20 Aug 2026 | `ml/corpus.py` runs `app.extract.entities` rather than its own patterns (§7D) | It held a second, weaker definition of an unredacted identifier and had already fallen behind the app. The gate now inherits the whole pipeline and cannot drift. |
| 20 Aug 2026 | **F10 admin scoped to feedback, metrics and audit**; content and version management dropped for now (§6) | F7 does not exist, so a content editor would administer nothing, and rule/model versions are code constants — a table would duplicate state and create two sources of truth. Revisit content management with F7, and version tables when activation actually moves into the database. |
| 20 Aug 2026 | Administrators cannot read scan text, even masked (§5, §7G) | Synopsis §2.5 says administrators cannot freely browse private scans. The review queue carries evidence codes, band and category but no excerpt. Reviewing a case that genuinely needs the content requires an explicit consent flow, not a wider default. |
